...
Node::Node(
  ...
  wall_timers_.push_back(node_handle_.createWallTimer(
      ::ros::WallDuration(robot_pose_to_cloud_publish_period_sec_),
      &Node::PublishTrajectoryStatesToCloud, this));
  wall_timers_.push_back(node_handle_.createWallTimer(
...
void Node::PublishTrajectoryStates(const ::ros::WallTimerEvent& timer_event) {
    map_robot_odom_high_rate_publisher_ = node_handle_.advertise<nav_msgs::Odometry>("robot_loc_highrate",10);
    carto::common::MutexLocker lock(&mutex_);
  for (const auto& entry : map_builder_bridge_.GetTrajectoryStates()) {
    ...
    // trajectory_state存放的是local_to_map信息
    // extrapolator存放的是tracking_to_local的信息
...
void Node::PublishTrajectoryStatesToCloud(const ::ros::WallTimerEvent& timer_event) {
